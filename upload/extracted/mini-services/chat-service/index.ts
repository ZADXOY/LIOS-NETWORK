...
// ---------- Predefined game channels (Raid Planning moved to legions) ----------
const CHANNELS: ChannelDef[] = [
  { id: 'general', name: 'General', description: 'Main chat for all survivors', icon: '🏝️', accent: 'emerald' },
  { id: 'trading', name: 'Trading Post', description: 'Buy, sell & trade loot', icon: '💱', accent: 'amber' },
  { id: 'pvp', name: 'PvP Arena', description: 'Find duel partners and rivals', icon: '⚔️', accent: 'rose' },
  { id: 'guilds', name: 'Guild Recruitment', description: 'Recruit or join a guild', icon: '🛡️', accent: 'sky' },
  { id: 'help', name: 'Help & Guides', description: 'Tips, quests and strategies', icon: '📜', accent: 'violet' },
...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('join-channel', (data: { channelId: string }) => {
    ...
    const joinMsg = createSystemMessage(channel.id, `${user.username} joined the channel`)
  ...
  // ---------- LEGION RECRUITMENT ----------
  // Leader writes a recruitment pitch → broadcast to the public Guild Recruitment channel
  socket.on('legion:recruit', (data: { reason: string }) => {
    ...
    socket.emit('legion:recruit-posted', {
      message: 'Your recruitment post was published to the Guild Recruitment channel.',
    })

    // System message in legion chat
    const sysMsg = createSystemMessage(
      `legion:${legion.id}`,
      `Leader ${user.username} posted a recruitment message to the Guild Recruitment channel`,
    )
    pushLegionHistory(legion, sysMsg)
    emitLegionMessage(legion, sysMsg)
  ...
  socket.on('admin:approve-legion', (data: { legionId: string }) => {
    ...
    const sysMsg = createSystemMessage(
      `legion:${legion.id}`,
      `✅ Legion has been approved by admin. You can now recruit members and post to the Guild Recruitment channel.`,
    )
    pushLegionHistory(legion, sysMsg)
    emitLegionMessage(legion, sysMsg)
...
httpServer.listen(PORT, () => {
  console.log(`Last Island of Survival chat service running on port ${PORT}`)
